...
contract testLED {
    // enum LEDState { Off, RED, GREEN, BLUE }
    // LEDState public currentState = LEDState.Off;
    ...
    // event LEDStateChanged(LEDState newState);
    event VotingCycleReset(uint256 newEndTime);
    // event VotingCycleEnded(LEDState finalState); // New event for cycle end
    event TimeCalculated(uint256 timeRED, uint256 timeGREEN, uint256 timeBLUE);

    constructor() {
        cycleEndTime = block.timestamp + 0.5 minutes;
    }
    ...
    timeRED = (QuadraticVotingRed * 60 * precision / TotalQuadraticVotesSquared);
    timeGREEN = (QuadraticVotingGreen * 60 * precision / TotalQuadraticVotesSquared);
    timeBLUE = (QuadraticVotingBlue * 60 * precision / TotalQuadraticVotesSquared);
